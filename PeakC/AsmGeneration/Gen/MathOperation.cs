...
namespace Peak.AsmGeneration
{
    static class MathOperation
    {
        ...
        private static GenResult emitDiv(GenResult leftRes, GenResult rightRes, Token op, SymbolTable st)
        {
            ...
            st.Emit("xor rdx, rdx");
            st.MemoryAllocator.MoveToRegister(left, RegisterName.rax);
            st.MemoryAllocator.Block(RegisterName.rax);

            // overwrite second operand in register
            var divOp = st.MemoryAllocator.MoveToAnyRegister(rightRes).Register.ToString();
